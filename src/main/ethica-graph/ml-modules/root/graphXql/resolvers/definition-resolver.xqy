...
declare function gxqlr:definition-entity-resolver($var-map as map:map) as element(*, gxql:Definition)?
{
    let $uri :=
...
declare function gxqlr:definition-field-resolver($field-name as xs:string) as xdmp:function
{
    ...
    else if ($field-name eq 'descendants') then xdmp:function(xs:QName('gxqlr:definition-descendants-resolver'))
    else
        fn:error((), 'FIELD RESOLVER EXCEPTION', ("500", "Internal server error", "unsupported field: "||$field-name))
};

declare function gxqlr:definition-descendants-resolver($item as element(*, gxql:Definition), $var-map as map:map)
{
