...
declare function gxqlr:proposition-lemme-corollary-entity-resolver($var-map as map:map) as element(*, gxql:PropositionLemmeCorollary)?
{
    let $uri :=
        ...
        then
            cts:uris('', (), cts:and-query((
              cts:element-value-query(xs:QName('eth:type'), 'Proposition-Lemme-Corollary'),
    ...
    return
        element gxql:propositionLemmeCorollary
        {
...
declare function gxqlr:proposition-lemme-corollary-field-resolver($field-name as xs:string) as xdmp:function
{
    ...
    else if ($field-name eq 'propositionLemmeNumber') then xdmp:function(xs:QName('gxqlr:plc-proposition-lemme-number-resolver'))
    else if ($field-name eq 'propositionNumber') then xdmp:function(xs:QName('gxqlr:plc-proposition-number-resolver'))
    else
        fn:error((), 'FIELD RESOLVER EXCEPTION', ("500", "Internal server error", "unsupported field: "||$field-name))
};

declare function gxqlr:plc-proposition-lemme-number-resolver($item as element(*, gxql:PropositionLemme), $var-map as map:map)
{
    fn:doc($item/uri/fn:string())//eth:coordinates/eth:proposition-lemme-number/string()
};

declare function gxqlr:plc-proposition-number-resolver($item as element(*, gxql:PropositionLemme), $var-map as map:map)
{
