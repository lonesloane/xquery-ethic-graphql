...
declare function gxqlr:postulate-entity-resolver($var-map as map:map) as element(*, gxql:Postulate)?
{
    let $uri :=
        ...
        then
            ...
            cts:uris('', (), cts:and-query((
              cts:element-value-query(xs:QName('eth:type'), 'Postulate'),
...
declare function gxqlr:postulate-list-resolver($var-map as map:map) as element(*, gxql:Postulate)*
{
    let $partNumber := map:get($var-map, 'partNumber')=>xs:string()
    let $uris :=
        cts:uris('', (), cts:and-query((
    ...
    for $uri in $uris
    order by xs:int(fn:doc($uri)/eth:postulate/eth:coordinates/eth:ordinal) ascending
    return
        element gxql:postulate
        {
            element gxql:uri {$uri}
...
declare function gxqlr:postulate-field-resolver($field-name as xs:string) as xdmp:function
{
