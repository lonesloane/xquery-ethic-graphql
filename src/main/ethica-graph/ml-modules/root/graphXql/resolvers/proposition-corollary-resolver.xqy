...
declare function gxqlr:proposition-corollary-entity-resolver($var-map as map:map) as element(*, gxql:PropositionCorollary)?
{
    let $uri :=
        ...
        then
            cts:uris('', (), cts:and-query((
              cts:element-value-query(xs:QName('eth:type'), 'Proposition-Corollary'),
    ...
    return
        element gxql:propositionCorollary
        {
            element gxql:uri {$uri}
        }
};

declare function gxqlr:proposition-corollary-field-resolver($field-name as xs:string) as xdmp:function
{
    if ($field-name eq $gxqlr:ETHIC-ITEM-FIELDS) then
        ...
        xdmp:function(xs:QName('gxqlr:proposition-corollary-propositionNumber-resolver'))
...
declare function gxqlr:proposition-corollary-propositionNumber-resolver($item as element(*, gxql:PropositionCorollary), $var-map as map:map)
{
    fn:doc($item/uri/fn:string())/eth:proposition-corollary/eth:coordinates/eth:proposition-number/string()
};
