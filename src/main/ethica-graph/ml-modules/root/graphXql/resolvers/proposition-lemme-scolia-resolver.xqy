...
declare function gxqlr:proposition-lemme-scolia-entity-resolver($var-map as map:map) as element(*, gxql:PropositionLemmeScolia)?
{
    let $uri :=
        ...
        then
            cts:uris('', (), cts:and-query((
              cts:element-value-query(xs:QName('eth:type'), 'Proposition-Lemme-Scolia'),
    ...
    return
        element gxql:propositionLemmeScolia
        {
            element gxql:uri {$uri}
        }
};

declare function gxqlr:proposition-lemme-scolia-field-resolver($field-name as xs:string) as xdmp:function
{
    ...
    else if ($field-name eq 'propositionLemmeNumber') then xdmp:function(xs:QName('gxqlr:pls-proposition-lemme-number-resolver'))
    else if ($field-name eq 'propositionNumber') then xdmp:function(xs:QName('gxqlr:pls-proposition-number-resolver'))
    else
        fn:error((), 'FIELD RESOLVER EXCEPTION', ("500", "Internal server error", "unsupported field: "||$field-name))
};

declare function gxqlr:pls-proposition-lemme-number-resolver($item as element(*, gxql:PropositionLemme), $var-map as map:map)
{
    fn:doc($item/uri/fn:string())//eth:coordinates/eth:proposition-lemme-number/string()
};

declare function gxqlr:pls-proposition-number-resolver($item as element(*, gxql:PropositionLemme), $var-map as map:map)
{
