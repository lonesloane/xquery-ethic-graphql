...
declare function gxqlr:proposition-scolia-entity-resolver($var-map as map:map) as element(*, gxql:PropositionScolia)?
{
    let $uri :=
        ...
        then
            cts:uris('', (), cts:and-query((
              cts:element-value-query(xs:QName('eth:type'), 'Proposition-Scolia'),
    ...
    return
        element gxql:propositionScolia
        {
...
declare function gxqlr:proposition-scolia-field-resolver($field-name as xs:string) as xdmp:function
{
    ...
    else if ($field-name eq 'propositionNumber') then xdmp:function(xs:QName('gxqlr:proposition-scolia-propositionNumber-resolver'))
    else
        fn:error((), 'FIELD RESOLVER EXCEPTION', ("500", "Internal server error", "unsupported field: "||$field-name))
};

declare function gxqlr:proposition-scolia-propositionNumber-resolver($item as element(*, gxql:PropositionScolia), $var-map as map:map)
{
    fn:doc($item/uri/fn:string())/eth:proposition-scolia/eth:coordinates/eth:proposition-number/string()
};
